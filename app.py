...
async def startup_event():
    """アプリケーション起動時の初期化処理"""
    logger.info("Idea Spark API starting up...")
    ...
    try:
        ...
        if db_result["status"] == "connected":
            ...
            logger.info(f"pgvector enabled: {db_result.get('pgvector_enabled', False)}")
        else:
            logger.error(f"Database connection failed: {db_result.get('error')}")
    except Exception as e:
    ...
    logger.info("Idea Spark API startup complete")

@app.on_event("shutdown")
async def shutdown_event():
    """アプリケーション終了時の処理"""
    logger.info("Idea Spark API shutting down...")
